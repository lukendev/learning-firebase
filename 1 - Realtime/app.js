...
function criarCard() {
    ...
    //push(): cria um id unico e insere os dados dentro desse unique id
    // ref.push(card).then(snapshot => {
    //     // adicionaCardATela(card, snapshot.key)
    // })

    //USANDO FETCH PARA ADICIONAR CARD
    fetch('https://learning-realtimedatabase.firebaseio.com/card.json', {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ref.once('value').then(snapshot => {
        ...
        //acessa um nó filho
        // console.log('child', snapshot.child('-MBQA_lqlEAQMTdCgYwj').val())

        //checa se existe algo no snapshot
        // console.log('exists()', snapshot.exists())

        // // se existe o filho passado na url
        // console.log('hasChild() nome', snapshot.hasChild('-MBQA_lqlEAQMTdCgYwj/nome'))
        // console.log('hasChild() comentario', snapshot.hasChild('-MBQA_lqlEAQMTdCgYwj/comentario'))

        // se existe algum filho no nó
        // console.log('hasChildren()', snapshot.child('-MBQA_lqlEAQMTdCgYwj').hasChildren())
    ...
    // ORDENAÇÃO
    // .orderByChild('filho'): Ordena pela propriedade filho passada como parâmetro
    // .orderByKey(): Ordenar pela chave dos nós
    ...
    // .startAt(25).endAt(35)
    // .startAt(): Traz valores cujo valor passado na query comece no valor passado por parâmetro no método.
    // .endAt(): Traz valores cujo valor passado na query vá até o valor passado por parâmetro no método
    ...
    // limitToLast(Numebr): Retorna apenas os ultimos valores de acordo com o numero passado por parametro
    ...
    fetch('https://learning-realtimedatabase.firebaseio.com/card.json').then(res => res.json()).then(res => {
        for(var key in res) {
